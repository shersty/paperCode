...
public class IDS_KGIRAV01 {
    int Un[][] = null;
    int Ux[] = null;
    int flag = 0;
    double tempUUUxRelative;

    public IDS_KGIRAV01(int a[][], int b[]) {
        Un = a;
        Ux = b;
    }
    ...
    //构造函数，给两个实例变量赋值
    public IDS_KGIRAV01(ArrayList<ArrayList<Integer>> orgData, ArrayList<Integer> addData) {
        setUn(orgData);
        setUx(addData);
    }
    ...
    public double get_GP(int[][] Un, List<Integer> P) {
        ...
        double gp = Arith.div(sum, Math.pow(n, 2));
        gp = Arith.round(gp, 9);
...
//        double UUUxRelative = (Math.pow(n, 2) * (get_GP(Un, P) - get_GP(Un, PUD)) / Math.pow(n + 1, 2) + 2 * (sum(PTC) - sum(PTCUD)) / Math.pow(n + 1, 2));///Math.pow(n+1,2);
        double part1 = Arith.mul(Math.pow(n, 2), Arith.sub(get_GP(Un, P), get_GP(Un, PUD)));
        double part2 = Arith.mul(2, (sum(PTC) - sum(PTCUD)));
        double part3 = Arith.div(part1 + part2, Math.pow(n + 1, 2));
        part3 = Arith.round(part3, 9);//相对约简取 9位小数
//        double part3 = part1 + part2;
    ...
    public double get_UUUxRelativeC(int[][] Un, int[] Ux, List<Integer> P) {
        ...
        double UUUxRelative;
        if (flag == 0) {
//            UUUxRelative = (Math.pow(n, 2) * (get_GP(Un, P) - get_GP(Un, PUD)) / Math.pow(n + 1, 2) + 2 * (sum(PTC) - sum(PTCUD)) / Math.pow(n + 1, 2));///Math.pow(n+1,2);
            double part1 = Arith.mul(Math.pow(n, 2), Arith.sub(get_GP(Un, P), get_GP(Un, PUD)));
            double part2 = Arith.mul(2, (sum(PTC) - sum(PTCUD)));
            double part3 = Arith.div(part1 + part2, Math.pow(n + 1, 2));
            part3 = Arith.round(part3, 9);//相对约简取9位小数
//            double part3 = part1 + part2;
            tempUUUxRelative = part3;
            flag++;
            return part3;
        }
        UUUxRelative = ((Math.pow(n, 2) *  Arith.sub(get_GP(Un, P), get_GP(Un, PUD))) + 2 * (sum(PTC) - sum(PTCUD))) / Math.pow(n + 1, 2);
//        tempUUUxRelative = Arith.round(tempUUUxRelative, 4);
//        System.out.println("+++++++++++++++++++++++++++++++");
        double test1 = Arith.sub(get_GP(Un, P), get_GP(Un, PUD));
        double test2 = tempUUUxRelative;
        System.out.println("重新计算的知识粒度:" + test1);
        System.out.println("重用上次的知识粒度:" + test2);
        double part1 = Arith.mul(Math.pow(n, 2), tempUUUxRelative);
        double part2 = Arith.mul(2, (sum(PTC) - sum(PTCUD)));
        double part3 = Arith.div(part1 + part2, Math.pow(n + 1, 2));
        part3 = Arith.round(part3, 9);//相对约简取9位小数
//        double part3 = part1 + part2;
        tempUUUxRelative = part3;
        System.out.println("此次计算的知识粒度：" + tempUUUxRelative);
        System.out.println("好东西:" + Arith.round(UUUxRelative, 9));
        return part3;
    }
...
//        double UUUxSigOuter = (Math.pow(n, 2) * get_SigOuter(Un, a, P) + 2 * (sum(PTC) - sum(PTCUD)) - 2 * (sum(PTPUa) - sum(PTPUaUD)));///((n+1)^2);
        double UUUxSigOuter = (Math.pow(n, 2) * get_SigOuter(Un, a, P) + 2 * (sum(PTC) - sum(PTCUD)) - 2 * (sum(PTPUa) - sum(PTPUaUD))) / Math.pow(n + 1, 2);

        double part1 = Arith.mul(Math.pow(n, 2), get_SigOuter(Un, a, P));
        double part2 = Arith.mul(2, (sum(PTC) - sum(PTCUD)) - (sum(PTPUa) - sum(PTPUaUD)));
        double part3 = Arith.div(part1 + part2, Math.pow(n + 1, 2));
//        double part3 = part1 + part2;
        return part3;
    }
    ...
    public List<Integer> KGIRA(int Un[][], List<Integer> SReduct, int Ux[]) {
        ...
        C.addAll(P);
        System.out.println("-------------------------------");
        double new_GPRelativeC = get_UUUxRelativeC(Un, Ux, C);//计算GP_UUUx(D|C)
        System.out.println("GP_UUUx(D|C)-V2:" + new_GPRelativeC);
        double new_GPRelativeB = get_UUUxRelative(Un, Ux, B);//计算GP_UUUx(D|B)
        System.out.println("GP_UUUx(D|B)-V2:" + new_GPRelativeB);
        System.out.println("-------------------------------");
