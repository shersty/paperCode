...
//        System.out.println("B:" + P);
        ...
        if(flag == 0 ||!P.equals(tempB)){//如果 P 和 B 相等
...
//        }
    ...
    public List<Integer> KGIRA(int Un[][], List<Integer> SReduct, int Ux[]) {
        ...
        C.addAll(P);
        double new_GPRelativeC = get_UUUxRelativeC(Un, Ux, C);//计算GP_UUUx(D|C)  使用上一次的知识粒度计算增量的知识粒度，缩短计算时间
        double new_GPRelativeB = get_UUUxRelativeB(Un, Ux, B);//计算GP_UUUx(D|B)   根据 B的情况来利用上一次的计算结果缩短计算时间
...
//        while (new_GPRelativeC - get_UUUxRelative(Un, Ux, B) > 0.0000000000001 || get_UUUxRelative(Un, Ux, B)  - new_GPRelativeC > 0.0000000000001) {
        while (new_GPRelativeC != get_UUUxRelative(Un, Ux, B)) {
            ...
            temp.addAll(P);
            temp.removeAll(B);// temp = P - B; 遍历B以外的集合，找出外部显著性最大的
